...
async fn go() -> Result<(), MhkError> {
    ...
    let outfs = filesystem::LoggingFilesystem::new("w", filesystem::LocalFilesystem::new("./local/"));
    
    let map = MhkMap::new(fs, [
        (riven::Stack::A, vec!["a_Data.MHK", "a_Sounds.MHK"]),
        (riven::Stack::B, vec!["b_Data.MHK", "b2_data.MHK", "b_Sounds.MHK"]),
        (riven::Stack::G, vec!["g_Data.MHK", "g_Sounds.MHK"]),
        (riven::Stack::J, vec!["j_Data1.MHK", "j_Data2.MHK", "j_Sounds.MHK"]),
