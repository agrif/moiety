...
use crate::mhk::{deserialize_from, deserialize_u16_table_from};
use crate::MhkError;
...
pub async fn deserialize_handlers<'a, R>(reader: &'a R, pos: &'a mut u64) -> Result<std::collections::HashMap<Event, Vec<Command>>, MhkError> where R: AsyncRead {
...
pub fn deserialize_commands<'a, R>(reader: &'a R, pos: &'a mut u64) -> Fut<'a, Result<Vec<Command>, MhkError>> where R: AsyncRead {
    fut!({
        let count: u16 = await!(deserialize_from(reader, pos))?;
