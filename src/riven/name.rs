...
use crate::mhk::{deserialize_from, deserialize_vec_from};
...
impl<R> FormatFor<R, Resource<Vec<Name>>> for MhkFormat where R: AsyncRead {
    fn convert<'a>(&'a self, input: R) -> Fut<'a, Result<Vec<Name>, MhkError>> where R: 'a {
        fut!({
            let mut pos = 0;
