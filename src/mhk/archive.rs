...
impl<R> MhkArchive<R> where R: AsyncRead {
    pub async fn new(unbuffered_handle: R) -> Result<Self, MhkError> {
        ...
        let _name_list_offset: u16 = await!(deserialize_from(&handle, &mut pos))?;

        // read in the type table
        let type_table: Vec<TypeTableEntry> = await!(deserialize_u16_table_from(&handle, &mut pos))?;
        ...
        let mut resource_tables: Vec<Vec<ResourceTableEntry>> = Vec::with_capacity(type_table.len());
        for entry in &type_table {
            pos = rsrc.resource_dir_offset as u64 + entry.resource_table_offset as u64;
            resource_tables.push(await!(deserialize_u16_table_from(&handle, &mut pos))?);
        ...
        let file_table: Vec<FileTableEntry> = await!(deserialize_u32_table_from(&handle, &mut pos))?;

        // convert the file table into something more useful for us
        let files: Vec<FileInfo> = file_table.iter().map(|e| FileInfo {
        ...
        for (i, entry) in type_table.iter().enumerate() {
            ...
            for rentry in resource_table {
                if rentry.file_table_index as usize - 1 >= files.len() {
                    return Err(MhkError::InvalidFormat("bad file table index"));
                }
