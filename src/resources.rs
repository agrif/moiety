...
impl<M, F, E> Resources<M, F, E> where M: ResourceMap, F: Format<M::Handle>, E: From<M::Error> + From<F::Error> {
    ...
    pub async fn write_resource_to<'a, R: ResourceType + 'a, Mw: 'a, Fw: 'a, Ew: 'a>(&'a self, other: &'a mut Resources<Mw, Fw, Ew>, stack: M::Stack, typ: R, id: u16) -> Result<(), ConvertError<E, Ew>> where F: FormatFor<M::Handle, R>, Mw: ResourceMapWrite<Stack=M::Stack>, Fw: FormatWriteFor<M::Handle, R, F>, Ew: From<Mw::Error> + From<Fw::WriteError>, E: failure::Fail, Ew: failure::Fail, M::Stack: Clone {
        let handle = await!(self.map.open_raw(stack.clone(), typ, id)).map_err(|e| ConvertError::Read(e.into()))?;
        ...
        await!(other.map.write_raw(stack, typ, id, &data)).map_err(|e| ConvertError::Write(e.into()))
    }
